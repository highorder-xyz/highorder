...
fn parse_primary(it: &mut Parser) -> Result<WithSpan<Expr>, ()> {
    let tc = it.advance();
    match &tc.value {
        &Token::Null => Ok(WithSpan::new(Expr::Null, tc.span)),
...
mod tests {
    ...
    fn test_unary() {
        use make::*;
        use help::assert2;
        assert2("-null", uo(UnaryOperator::Minus, 0..1, Expr::Null, 1..5), 0..5);
        assert2("!null", uo(UnaryOperator::Bang, 0..1, Expr::Null, 1..5), 0..5);
        assert2("!!null", uo(UnaryOperator::Bang, 0..1, uo(UnaryOperator::Bang, 1..2, Expr::Null, 2..6), 1..6), 0..6);
        assert2("!-null", uo(UnaryOperator::Bang, 0..1, uo(UnaryOperator::Minus, 1..2, Expr::Null, 2..6), 1..6), 0..6);
        assert2("-!null", uo(UnaryOperator::Minus, 0..1, uo(UnaryOperator::Bang, 1..2, Expr::Null, 2..6), 1..6), 0..6);
    }
    ...
    fn test_list() {
        ...
        let null = ws(Expr::Null, 4..8);
        let expr = ws(Expr::List(vec![num, null]), 0..9);
        assert("[1, null]", expr);
