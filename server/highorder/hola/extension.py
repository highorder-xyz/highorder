...
class HolaServiceRegister:
    ...
    def register(cls, svc_class):
        ...
        if name in cls._services:
            raise Exception(
                f"Service With Name {name} already registered. {svc_class}."
            )

        cls._services[name] = {"svc_class": svc_class, "registered": True}
    ...
    async def call(cls, name, args):
        ...
        if isinstance(func, dict) or not callable(func):
            raise Exception(
                f'Service name {name} must have func name, like "module.function" format.'
            )
