...
from highorder.hola.extension import (
    ...
    ShowAlertCommand,
    ShowAlertCommandArg,
    ExtensionCallSucceed,
    ExtensionCallSucceedArg,
    ExtensionCallFailed,
    ExtensionCallFailedArg,
    AutoList,
)
...
class UserService:
    ...
    def get_data(self):
        return UserData(user_id=self.model.user_id, user_name=self.model.user_name)

    def get_data_dict(self):
        return dict(user_id=self.model.user_id, user_name=self.model.user_name)
...
class UserAuthService:
    ...
    @classmethod
    async def update_user(cls, app_id, user_id, password = None):
        if not password:
            return
        user_auth = await UserAuth.get_or_none(app_id = app_id, user_id = user_id)
        user_auth.password_hash = UserAuthService.get_password_hash(
            password, user_auth.salt
        )
        async with in_transaction(DB_NAME):
            await user_auth.save()
...
class SocialAccountService:
    ...
    async def login_weixin(cls, app_id, user_id, code):
        ...
        session = await new_user.new_session(session_data={"weixin_login": True})
        # udpate profile
...
class AccountServiceExtension:
    ...
    async def login_name_password(cls, args):
        ...
        if not name or not password:
            raise Exception(
                f'name and password must be provided, but go name: "{name}", password: "{password}"'
            )
        info = args["__info__"]
        app_id = info["app_id"]
            ...
            commands.add(
                SetSessionCommand(
                    args=SetSessionCommandArg(
                        session=session_data, user=user.get_data_dict()
                    )
                )
            )
        else:
