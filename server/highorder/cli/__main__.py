...
def create_application_id():
    generated = ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(18))
    return 'AP{}'.format(generated)

def random_str(prefix='', number=10):
    generated = ''.join(random.choice(string.ascii_lowercase + string.digits) for _ in range(number))
...
class AppCreator:
    ...
    def create_app(self, folder):
        ...
        app_description = typer.prompt("Please decribe you project",
            default="This app is make for ....", show_default=True)
        admin_user = typer.prompt("What's admin name?", default="admin", show_default=True)
        salt = random_str('', 4)
